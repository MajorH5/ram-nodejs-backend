...
class UserDatabase {
    ...
    static async createPasswordResetToken (email) {
        ...
        const user = userResult[0];
        const lastAttempt = user.lastResetRequest;

        if (lastAttempt && new Date() - lastAttempt < RESET_PASSWORD_THROTTLE_TIME) {
        ...
        const lastSetQuery = `UPDATE users SET lastResetRequest = ? WHERE email = ?`;
        const lastSetResult = await Database.query(lastSetQuery, [new Date(), email]);
