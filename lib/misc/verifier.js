...
class Verifier {
    ...
    static validateComment (comment) {
        ...
        if (comment.length < 1) return 'Comment too short';
        if (comment.length > 2048) return 'Comment too long';

        return true;
    }

    static validCommentStatus (status) {
        if (typeof status !== 'string') return 'Status must be a string';
        if (status !== 'like' && status !== 'dislike'  && status !== 'none') return 'Invalid status';

        return true;
    }

    static validateReportReason (reason) {
        if (!Array.isArray(reason)) return 'Reason must be a array';
        ...
        if (reason.length === 0) return 'No report reason provided';
        if (reason.length > options.length) return 'Too many report reasons';
        if (reason.some(r => typeof r !== 'string')) return 'Report reasons must be strings';
        if (reason.some(r => !options.includes(r))) return 'Invalid report reason';
